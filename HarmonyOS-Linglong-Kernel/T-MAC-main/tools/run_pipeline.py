...
def run_hdc_command(command, pwd):
    """Run hdc (Harmony Device Connector) command. Falls back to adb if hdc not found."""
    hdc = shutil.which('hdc') or shutil.which('hdb')
    if not hdc:
        # fallback to adb
        return run_adb_command(command, pwd)
    new_command = [hdc]
    if FLAGS.adb_serial:
        new_command.append(f'-s {FLAGS.adb_serial}')
    new_command = new_command + command
...
def convert_to_hiai_step():
    """Convert GGUF/ggml model to HiAI format if device is deveco/hiai.

    This step requires an external converter tool and the environment
    variable `HIAI_CONVERTER` to be set to a command template.
    """
    if FLAGS.device not in ("deveco", "hiai"):
        print("Skipping HiAI conversion: device is not deveco/hiai")
        return

    model_dir = str(FLAGS.model_dir).rstrip('\\').rstrip('/')
    script = os.path.join(ROOT_DIR, "tools", "convert_to_hiai.py")
    command = ["python", script, model_dir, "--out-format", "om"]
    # allow specifying an explicit output filename
...
def cmake_llamacpp():
    ...
    # Support Android, DevEco and HiAI-enabled DevEco (hiai) using NDK/toolchain flow
    if FLAGS.device in ("android", "deveco", "hiai", "harmony"):
        try:
            ndk_home = FLAGS.ndk_home or os.environ.get("NDK_HOME")
            harmony_root = os.environ.get("HARMONY_SDK_ROOT") or os.environ.get("DEVECO_SDK_ROOT")
            # If Harmony SDK is present and device targets Harmony, prefer its toolchain
            if FLAGS.device in ("deveco", "harmony") and harmony_root:
                # try harmony-specific toolchain file if exists
                toolchain = os.path.join(harmony_root, "build", "cmake", "ohos.toolchain.cmake")
                if os.path.exists(toolchain):
                    command.append(f"-DCMAKE_TOOLCHAIN_FILE={toolchain}")
                else:
                    # fallback to android NDK if set
                    if ndk_home:
                        command.append(f"-DCMAKE_TOOLCHAIN_FILE={ndk_home}/build/cmake/android.toolchain.cmake")
                    else:
                        raise KeyError("Missing NDK_HOME or HARMONY_SDK_ROOT. Please set NDk_HOME or HARMONY_SDK_ROOT")
            else:
                if not ndk_home:
        ...
        # If targeting HiAI, try to inject HiAI SDK root to CMake so native parts
        # can link against HiAI libraries if provided via environment variable.
        if FLAGS.device == "hiai":
            hiai_root = os.environ.get("HIAI_SDK_ROOT") or os.environ.get("HIAI_ROOT")
            if hiai_root:
                command.append(f"-DHIAI_SDK_ROOT={hiai_root}")
                command.append(f"-DCMAKE_PREFIX_PATH={hiai_root}")
            else:
                print("Warning: HIAI_SDK_ROOT not set; HiAI-specific linking will be skipped.")
    elif is_win():
        if is_arm():
...
def run_inference():
    ...
    # Support pushing/running on Android, DevEco, HiAI and Harmony devices
    if FLAGS.device in ("android", "deveco", "hiai", "harmony"):
        remote_bin_path = os.path.join(FLAGS.remote_dir, "bin")
        # push binaries
        command = ['push', os.path.join(build_dir, "bin"), FLAGS.remote_dir]
        if FLAGS.device in ("deveco", "harmony"):
            run_hdc_command(command, build_dir)
        else:
...
def run_llama_bench():
    ...
    if is_win():
        ...
        main_path = os.path.join(build_dir, "bin", "llama-bench")
    prompt = 256
    # TODO: verify on Android and DevEco
    # Support Android/Harmony/DevEco/HiAI devices
    if FLAGS.device in ("android", "deveco", "hiai", "harmony"):
        remote_bin_path = os.path.join(FLAGS.remote_dir, "bin")
        command = ['push', os.path.join(build_dir, "bin"), FLAGS.remote_dir]
        if FLAGS.device in ("deveco", "harmony"):
...
STEPS = [
    ...
    ("Convert GGUF to HiAI format", convert_to_hiai_step),
