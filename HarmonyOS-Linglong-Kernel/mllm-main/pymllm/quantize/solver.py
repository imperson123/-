...
from ..ffi import MLLM_FIND_NUMPY_AVAILABLE, MLLM_FIND_TORCH_AVAILABLE, MLLM_FIND_MINDSPORE_AVAILABLE
    ...
    def stream_quantize(
        ...
        for k in left_name:
            ...
            if cast_left_2_fp32:
                ...
                elif MLLM_FIND_MINDSPORE_AVAILABLE and ms is not None and isinstance(ttt, getattr(ms, "Tensor", (object,))):
                    # try to cast MindSpore tensor to float32 or fallback to asnumpy
                    try:
                        # MindSpore has .astype
                        ttt = ttt.astype(ms.float32)
                    except Exception:
                        try:
                            if MLLM_FIND_NUMPY_AVAILABLE:
                                ttt = ttt.asnumpy().astype(np.float32)
                            else:
                                ttt = ttt.asnumpy()
                        except Exception:
                            # leave as-is
