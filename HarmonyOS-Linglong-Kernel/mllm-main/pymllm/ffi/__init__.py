...
MLLM_FIND_MINDSPORE_AVAILABLE = (
    importlib.util.find_spec("mindspore") is not None
    or importlib.util.find_spec("mindspore_lite") is not None
)
...
def is_mindspore_available() -> bool:
    return MLLM_FIND_MINDSPORE_AVAILABLE is not None


def from_mindspore(ms_tensor):
    """Convert a MindSpore tensor to an mllm.Tensor via numpy when possible.

    Strategy:
    - If the object has `asnumpy()` (full MindSpore), use it and call from_numpy.
    - If numpy is not available or conversion fails, try to call a potential
      `_ffi_api.from_mindspore` if exposed by the native layer.
    - If all fail, raise a ValueError.
    """
    # Strategy (best-effort, prefer zero-copy when possible):
    # 1. Try to obtain a DLPack capsule from MindSpore tensor and use TVM/DLPack path.
    # 2. Fallback to asnumpy() -> from_numpy.
    # 3. Final fallback: call native _ffi_api.from_mindspore if the native layer exposes it.
    # Note: To get optimal (zero-copy) behavior, install MindSpore with DLPack support.
    # See MindSpore install: https://www.mindspore.cn/install/#command
    # Lazy and defensive implementation below to avoid hard dependency.
    # 1) Try DLPack
    try:
        # common names for DLPack producer on Python objects
        dlpack_getters = [
            "to_dlpack",
            "__dlpack__",
            "toDLPack",
        ...
        dlpack_capsule = None
        for getter in dlpack_getters:
            if hasattr(ms_tensor, getter):
                try:
                    dlpack_capsule = getattr(ms_tensor, getter)()
                    break
                except Exception:
                    dlpack_capsule = None
        if dlpack_capsule is not None:
            # Try tvm_ffi-level conversion helper first (if available)
            conv = getattr(tvm_ffi, "from_dlpack", None)
            if conv is not None:
                return conv(dlpack_capsule)
            # Try _ffi_api.from_dlpack if native registered such function
            if hasattr(_ffi_api, "from_dlpack"):
                return _ffi_api.from_dlpack(dlpack_capsule)

    except Exception:
        # ignore and fallback to numpy path
        pass

    # 2) Try asnumpy -> from_numpy
    try:
        if hasattr(ms_tensor, "asnumpy"):
            arr = ms_tensor.asnumpy()
            return _ffi_api.from_numpy(arr)
        if MLLM_FIND_NUMPY_AVAILABLE:
            import numpy as _np

            arr = _np.array(ms_tensor)
            return _ffi_api.from_numpy(arr)
    except Exception:
        pass

    # 3) final fallback: if native ffi provides direct conversion
    if hasattr(_ffi_api, "from_mindspore"):
        return _ffi_api.from_mindspore(ms_tensor)

    raise ValueError(
        "Unable to convert MindSpore tensor to mllm.Tensor; please install numpy or a MindSpore build with DLPack support (see https://www.mindspore.cn/install/#command)"
    )
